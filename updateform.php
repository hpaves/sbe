		...
		if (isset($_POST['update'])){ /* Trükitähtedes kommenteeritud keeruline osa pärineb: http://stackoverflow.com/questions/23987602/deleting-mysql-rows-with-checkbox */
			...
			while ($row = mysqli_fetch_array($result)){ /* FETCH ARRAY */
			    ...
			    echo "<input class='form-control' type='text' name='Nimetus' maxlength='255' required value='".htmlspecialchars($row['Nimetus'])."'><br>";

			    echo "<label>Kirjeldus: </label><br>";
			    echo "<textarea class='form-control' name='Kirjeldus'>".htmlspecialchars($row['Kirjeldus'])."</textarea><br>";

			    echo "<label>Hind:</label>";
			    echo "<input class='form-control' type='number' name='Hind' min='0' required value='".htmlspecialchars($row['Hind'])."'><br><br>";

			    echo "<img class='updateform' src='images/" . htmlspecialchars($row['Pildinimi']) . "' alt='" . htmlspecialchars($row["id"]) . " '><br>";
